...
public class TrinomialModelSimulator extends StochasticProcessSimulator {
	 ...
	 * randomGenerator.getNextInteger() < thresholdUp, = 1 if thresholdUp <=
	 * randomGenerator.getNextInteger() <= thresholdDown = d if
	 * randomGenerator.getNextInteger() > thresholdDown. We ask indeed
	 * randomGenerator to generate a sequence of random numbers whose length is
	...
	private double[][] generateMovements() {
		...
		double thresholdUp = convertProbabilityUp();// when the simulated number is smaller than this, we have up
		double thresholdNotDown = convertProbabilityNotDown();// when the simulated number is bigger than this, we have
																// a
																// down
