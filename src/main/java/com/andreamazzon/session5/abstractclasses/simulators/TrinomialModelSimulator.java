...
/**
 * This class is used in order to simulate some paths of a trinomial model: discrete model for a stochastic
 ...
 * abstract class StochasticProcessSimulator, where the implementation of getRealizations() is given.
...
public class TrinomialModelSimulator extends StochasticProcessSimulator
{
	...
	//it is (1 + interestRate - decreaseIfDown - probabilityStayTheSame * (1 - decreaseIfDown))/(increaseIfUp - decreaseIfDown)
	...
	public TrinomialModelSimulator(double initialValue, double increaseIfUp, double decreaseIfDown, double probabilityStayTheSame,
			int seed, int lastTime, int numberOfSimulations){
		this(initialValue, increaseIfUp, decreaseIfDown, 0, probabilityStayTheSame, seed, lastTime, numberOfSimulations);
	}

	//overloaded constructor: if not specified, the seed is 1897
	public TrinomialModelSimulator(double initialValue, double increaseIfUp, double decreaseIfDown, double probabilityStayTheSame,
			int lastTime, int numberOfSimulations){
		this(initialValue, increaseIfUp, decreaseIfDown, 0, probabilityStayTheSame, 1897, lastTime, numberOfSimulations);
	}
	 ...
	 * by the maximum number that can be simulated. Thus, for a given time and a given simulation,
	 * if the simulated number is less that this we will return an up
	 */
	private long convertProbabilityUp() {
		//modulus is private, but we have the getter
		return Math.round(riskNeutralProbabilityUp * randomGenerator.getModulus());
	}

	/*
	 * here we consider the probability to have an up OR stay the same. Same thing as before, but we
	 * multiply riskNeutralProbabilityUp + probabilityStayTheSame
	 */
	private long convertProbabilityNotDown() {
		//modulus is private, but we have the getter
		return Math.round((riskNeutralProbabilityUp + probabilityStayTheSame) * randomGenerator.getModulus());
	}
	 ...
	 *  				  = 1 if thresholdUp <= randomGenerator.getNextInteger() <= thresholdDown
	 *                    = d if randomGenerator.getNextInteger() > thresholdDown.
	 *  We ask indeed randomGenerator to generate a sequence of random numbers whose length is equal
	 *  to finalTime*numerbOfSimulations.
	 */
	private double[][] generateMovements() {
		...
		long thresholdUp = convertProbabilityUp();//when the simulated number is smaller than this, we have up
		long thresholdNotDown = convertProbabilityNotDown();//when the simulated number is bigger than this, we have a down
	...
	/**
	 * It generates the realizations of the process S, depending on the one of the process M of ups and
	 * downs.
	 */
	/*
	 *  Note that the implementation is the same as in BinomialModel. One could then define another
	 *  abstract class MultinomialModelSimulator, extending StochasticProcessSimulator, where the
	 *  implementation of this method is given.
	 * Then TrinomialModelGenerator and BinomialModelGenerator inherit from that. Exercise?
	 */
	@Override
		...
		for (int timeIndex = 1; timeIndex <= lastTime; timeIndex ++) {
			for (int simulationIndex = 0; simulationIndex < numberOfSimulations; simulationIndex ++) {
				realizations[timeIndex][simulationIndex] =
						realizations[timeIndex - 1][simulationIndex] * movements[timeIndex - 1][simulationIndex];
			}
		}
	}
}
