...
/**
 * This class inherits from StochasticProcessUser, which provides methods to get the paths, the
 * simulated value at a given time and the average value at given time of a general stochastic process.
 * Here we consider a trinomial model, i.e., discrete model for a stochastic
 ...
 * Note that thanks to inheritance we just have to give the implementation of the method generateRealizations(),
 * since the one of the methods returning the paths, the simulated value at a given time and the average
 * value at given time, is already given in the base class.
 * This is also an example of composition, because the class has a field of type BinomialModelSimulator
...
public class TrinomialModelUser extends StochasticProcessUser{
	...
	TrinomialModelUser(double initialValue, double increaseIfUp, double decreaseIfDown, double interestRate,
	...
	TrinomialModelUser(double initialValue, double increaseIfUp, double decreaseIfDown, double probabilityStayTheSame,
	...
	//the realizations are generated with DELEGATION to the method of trinomialModelSimulator
	@Override
